...
이때 KDC (Key Distribution Center)에 TGT를 발급받는다.
...
이번 머신에서는 svc_tgs 즉, TGS 서비스 어카운트를 손에 넣었기 때문에 TGS를 요구할 수 있다.

TGS를 요구할 때는 유저 인증에 필요한 TGT와 접근하고 싶은 서비스를 나타내는 SPN (Service Principal Name)을 보낸다.

돌아오는 응답, TGS-REP (reply)로 부터 유저의 크레덴셜을 획득하는 것을 Kerberoasting이라고 한다.
...
이번 경우 Administrator의 SPN에 CIFS가 있는 것을 알 수 있다. CIFS는 smb 관련 권한을 가지고 있다.
